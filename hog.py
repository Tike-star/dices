...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if(opponent_score-player_score<3 and opponent_score-player_score>0): return True
    return False    
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while(score0<goal and score1<goal):
        while(who==0):
            ...
            if(not extra_turn(score0,score1) or score0>=goal): break #如果回合结束之后分数超过了目标分数或者不能进行额外回合，就不再进行下一轮
        while(who==1):
            num_rolls=strategy1(score1,score0)
            score1+=take_turn(num_rolls,score0,dice)
            say=say(score0,score1)
            if(not extra_turn(score1,score0) or score1>=goal): break #如果回合结束之后分数超过了目标分数或者不能进行额外回合，就不再进行下一轮
        who=other(who)        
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def f(score0,score1,rlast_score=last_score,rrunning_high=running_high):
        if(who==1):
            if(score1-rlast_score>rrunning_high):
                rrunning_high=score1-rlast_score
                print(rrunning_high,"point(s)! The most yet for Player 1")
            rlast_score=score1
        else:
            if(score0-rlast_score>rrunning_high):
                rrunning_high=score0-rlast_score
                print(rrunning_high,"point(s)! The most yet for Player 0")
            rlast_score=score0
        return announce_highest(who,rlast_score,rrunning_high)
    return f            
    # END PROBLEM 7
